...
def generate_psychedelic_animation():
    ...
    while running:
        ...
        # Create a dynamic colormap that shifts over time
        hue_shift = (i % 360) / 360.0
        colormap = mcolors.LinearSegmentedColormap.from_list('dynamic_hue', plt.cm.hsv(np.linspace(hue_shift, hue_shift + 1, 256)))

        # Map normalized values to colors using the dynamic colormap
